...
def p_logingui(pineappledevusername, pineappledevpassword):
    try:
        ...
        texto1 = tkinter.Label(login, text = "PineappleDev Login", bg="blue", fg="white")
        texto1.pack(fill = tkinter.X)
        texto2 = tkinter.Label(login, text = "", bg="black", fg="white")
        texto2.pack(fill = tkinter.X)
        texto3 = tkinter.Label(login, text = "Username", bg="black", fg="white")
        texto3.pack(fill = tkinter.X)

        cajaTexto1 = tkinter.Entry(login, width = 50, fg="white", bg="black")
        cajaTexto1.pack()

        texto4 = tkinter.Label(login, text = "", bg="black", fg="white")
        texto4.pack(fill = tkinter.X)

        texto5 = tkinter.Label(login, text = "Password", bg="black", fg="white")
        texto5.pack(fill = tkinter.X)

        cajaTexto2 = tkinter.Entry(login, show="*", width = 50, fg="white", bg="black")
        cajaTexto2.pack()

        texto5 = tkinter.Label(login, text = "", bg="black", fg="white")
        texto5.pack(fill = tkinter.X)

        def usernamei():
            pineappledevusernameinput = cajaTexto1.get()
            pineappledevpasswordinput = cajaTexto2.get()
            if str(pineappledevusernameinput) == str(pineappledevusername):
                if str(pineappledevpasswordinput) == str(pineappledevpassword):
                    print("PineappleDev > Successfully loged!")
                    texto5.configure(text="Successfully login!", fg="green")
                    login.destroy()
                else:
                    texto5.configure(text="Incorrect username/password!", fg="red")
            else:
                texto5.configure(text="Incorrect username/password!", fg="red")
            
        
        boton1 = tkinter.Button(login, text = "Login", command = usernamei, fg="white", bg="black")
        boton1.pack()

        login.mainloop()
    except:
        print("\nPineappleDev > A error ocurred during in the login GUI")
def p_systemterminalgui():
    try:
        ...
        login.title("PineappleDev System Terminal")
        login.configure(bg="black")

        texto1 = tkinter.Label(login, text = "PineappleDev System Terminal", bg="blue", fg="white")
        texto1.pack(fill = tkinter.X)
        texto2 = tkinter.Label(login, text = "", bg="black", fg="white")
        texto2.pack(fill = tkinter.X)

        commandinput = tkinter.Entry(login, width = 200, fg="white", bg="black")
        commandinput.pack()

        texto3 = tkinter.Label(login, text = "", bg="black", fg="white")
        texto3.pack(fill = tkinter.X)
        ...
        boton1 = tkinter.Button(login, text = "Execute", command = terminalcommand, fg="white", bg="black")
        boton1.pack()
